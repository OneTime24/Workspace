...
int main(){
    ...
    case 1:
    {
        ...
        for(int i=0;i<8;i++){
            ...
            printf("\nPlayer 1 : Select Box!\n");
            while(!validMove1){
                scanf("%d",&bx1);                                 //Using Switches to take one input from the player instead of the rows and colomns, That's why code became a little bit lenghty.
                switch(bx1){                                               //PLAYER 1
                    case 1:
                        if(arr[0][0]=='-'){             //if value at that index is equal to pre defined value swap.
                            arr[0][0]='X';
                            validMove1 = 1;
                        }else{
                            printf("Not Available Try Another one: ");  //If theres any other value then - like x or o then 
                        }
                        break;
                    case 2:
                        if(arr[0][1]=='-'){
        ...
        if(!gameOver){              //If no one won till the end, game won
