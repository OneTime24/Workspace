...
int main(){
	...
	//0 for closed
	//1 for open
	for(i=1;i<=m;i++){		// To assign 0 to all elements of array! basically all lockers are close
		num[i]=0;			
		togg[i]=0;
	}
	
	for(i=1;i<=n;i++){		//Loop throught each student
		for(j=i;j<=m;j+=i){		// Loop through each locker for each student, Increment by j+=i, to toggle every nth+jth locker depending on the student.
