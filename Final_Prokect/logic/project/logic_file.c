...
char* readFile(char *filename) {
    ...
    if (!fp) {
        ...
        printf("Please check if file exists and try again.\n");
        return NULL;
    }
    ...
    if (len == 0) {
        printf("Warning: File %s is empty or conversion failed\n", filename);
        fclose(fp);
        return NULL;
    }
...
int main() {
    ...
    while (1) {
        ...
        if (ch == 1) {
            ...
            // Load all files, skip ones that fail
            for (i = 0; i < n; i++) {
                raw[i] = readFile(filenames[i]);
                if (raw[i] == NULL) {
                    printf("Skipping %s due to error...\n\n", filenames[i]);
                } else {
                    valid_indices[valid_files] = i;
                    valid_files++;
                }
            }
            ...
            if(valid_files < 2) {
                ...
                printf("Only %d file(s) loaded successfully.\n", valid_files);
                // free loaded files
                for (i = 0; i < n; i++) {
                    if(raw[i] != NULL) free(raw[i]);
                }
            ...
            printf("Proceeding with comparison...\n\n");
